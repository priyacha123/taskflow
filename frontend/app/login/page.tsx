...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const lastWorkspace = data.workspaces[0]
      if (lastWorkspace) {
        setCurrentWorkspace(lastWorkspace)
        router.push(`/workspace/${lastWorkspace.slug}/projects`)
      } else {
    ...
    <div className="min-h-screen bg-white flex">
      <div className="hidden lg:flex w-1/2 bg-gray-950 flex-col justify-between p-10">
        ...
        <div className="space-y-6">
          {[
            { title: 'Kanban + list views', desc: 'Switch between kanban and list view per project.' },
            { title: 'Role-based access', desc: 'Owner, Admin, Member — full control over your workspace.' },
            { title: 'Multi-tenant', desc: 'Create multiple workspaces for different teams or clients.' }
          ].map(item => (
            <div key={item.title}>
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            ...
            <Link href="/register" className="text-gray-900 font-medium hover:underline">Sign up free</Link>
          </p>
        </div>
      </div>
    </div>
