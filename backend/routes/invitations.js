...
router.post('/:slug/invite', auth, workspaceMiddleware, requireRole('OWNER', 'ADMIN'), async (req, res, next) => {
  try {
    ...
    if (req.workspace.plan === 'FREE' && memberCount >= 10) {
      return res.status(403).json({
        error: 'FREE plan allows 10 members. Upgrade to PRO for unlimited members.'
      })
    }
